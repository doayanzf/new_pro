       ...
       <ul class='u1'>
           <li v-for="pro in goods" :key="pro.id" class="l1">
               <ul class='u2'>
                   ...
                   <li>{{pro.goodsName}}</li>
                   <li>￥{{pro.price}}<span>￥{{pro.marketPrice}}</span></li>
                   <li>加入购物车</li>
               </ul>
           </li>
       </ul>
...
export default {
    ...
    methods: {
     ...
     created() {
        this.axios.get('http://10.0.157.250:8888/tejiajingcai')
            .then(res => {
...
<style lang="css" scoped>
    ...
    .u2 li:nth-of-type(4) {
        background:yellow;
         height:.795756rem;
         width:3rem;
         border-radius:.530504rem;
         margin-left:.795756rem;
        line-height:.795756rem;
        font-size: .265252rem;
        text-align: center;
